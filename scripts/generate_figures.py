...
def quotation_corpus() -> dict:
    ...
    return {
        "assertions": quotes,
        "manuscripts": len(projects),
        "declaration_files": len(files),
        "unparseable_files": unparsed,
        "pins": dict(pins),
...
def metric_corpus() -> dict:
    ...
    for finding in (
        (yaml.safe_load(rp.read_text()) or {}).get("findings", []) if rp.is_file() else []
    ):
...
figures = {
    "quotation_corpus": quotation_corpus(),
