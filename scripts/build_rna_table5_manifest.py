            ...
            "evidence": [
                {
                    "kind": "metric",
                    "artifact": aid,
                    "name": f"{model} mean PS",
                    "reported": reported,
                    "pointer": "/results/mean_best_ps",
                }
            ],
        }
    )
    ...
    yaml.safe_dump(
        {
            ...
            "project": "rna-structure-awareness",
            "artifacts": artifacts,
            "claims": claims,
        },
        sort_keys=False,
...
print(
    f"  wrote {R / 'repro.yaml'}: {len(claims)} metric assertions over {len(artifacts)} artifacts"
)
if missing:
