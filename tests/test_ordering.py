...
def build(tmp_path, *, registration=Registration.CONFIRMATORY, note="", runs=(),
          pin_plan=True, plan_moved=False, results=RESULTS):
    ...
    if pin_plan:
        digest = (Digest(algorithm="sha256", value="0" * 64) if plan_moved
                  else Digest.of_file(plan))
        artifacts.append(ArtifactRef(id="plan", path=plan, digest=digest))
    return Manifest(
        project="p", artifacts=tuple(artifacts), runs=tuple(runs),
        claims=(Claim(id="c", text="t", registration=registration, registration_note=note,
                      evidence=(MetricEvidence(artifact="results", name="x",
                                               reported="3.2", pointer="/x"),)),))
...
def run(tmp_path, *, plan="plan", bind=True, body=RESULTS, **kw) -> RunRecord:
    outputs = (RunOutput(artifact="results", digest=digest_of(tmp_path, body)) if bind
...
def test_ordering_holds_across_timezones(tmp_path):
    ...
    started = datetime.datetime(2026, 8, 1, 9, 0,
...
def test_a_run_for_other_artifacts_does_not_order_this_claim(tmp_path):
    other = RunRecord(id="elsewhere", registered_plan="plan",
                      outputs=(RunOutput(artifact="something-else"),),
                      registered_at=REGISTERED, started_at=AFTER)
    assert check(build(tmp_path, runs=[other]))[1] is OrderingReason.NO_RUN_RECORD


def test_a_run_naming_no_plan(tmp_path):
    manifest = build(tmp_path, runs=[run(tmp_path, plan="", registered_at=REGISTERED,
                                         started_at=AFTER)])
...
def test_a_plan_whose_pin_is_broken_settles_nothing(tmp_path):
    ...
    manifest = build(tmp_path, plan_moved=True,
                     runs=[run(tmp_path, registered_at=REGISTERED, started_at=AFTER)])
    ordering, reason, detail = check(manifest)
    assert (ordering, reason) == (Ordering.UNCHECKED,
...
def test_an_output_named_but_not_bound_by_digest(tmp_path):
    ...
    manifest = build(tmp_path, runs=[run(tmp_path, bind=False, registered_at=REGISTERED,
                                         started_at=AFTER)])
    ordering, reason, detail = check(manifest)
...
def test_an_output_whose_bytes_are_not_the_ones_the_run_produced(tmp_path):
    manifest = build(tmp_path, runs=[run(tmp_path, body=json.dumps({"x": 9.9}),
                                         registered_at=REGISTERED, started_at=AFTER)])
...
@pytest.mark.parametrize("registration,note", [
...
def test_only_a_confirmatory_claim_is_ordered_against_a_plan(tmp_path, registration, note):
    manifest = build(tmp_path, registration=registration, note=note,
...
def test_the_weakest_authority_among_producing_runs_is_reported(tmp_path):
    strong = run(tmp_path, registered_at=REGISTERED, started_at=AFTER,
                 registration_authority=RegistrationAuthority.ZENODO)
    assert verify(build(tmp_path, runs=[strong])).claims[0].registration_authority is \
        RegistrationAuthority.ZENODO


def test_an_unqualified_timestamp_is_self_recorded(tmp_path):
    manifest = build(tmp_path, runs=[run(tmp_path, registered_at=REGISTERED,
                                         started_at=AFTER)])
    assert verify(manifest).claims[0].registration_authority is \
        RegistrationAuthority.SELF_RECORDED
...
def test_a_policy_can_require_more_than_a_self_recorded_timestamp(tmp_path):
    ...
    report = verify(build(tmp_path, runs=[run(tmp_path, registered_at=REGISTERED,
                                              started_at=AFTER)]))
    demanding = Policy(name="demanding",
...
def test_a_sufficient_authority_raises_no_violation(tmp_path):
    report = verify(build(tmp_path, runs=[
        run(tmp_path, registered_at=REGISTERED, started_at=AFTER,
            registration_authority=RegistrationAuthority.TRUSTED_TIMESTAMP)]))
    demanding = Policy(name="demanding",
                       minimum_registration_authority=RegistrationAuthority.OSF)
    assert not any(v.rule == "claim.registration_authority"
                   for v in demanding.assess(report).violations)
...
def test_an_unchecked_ordering_is_graded_by_profile_and_names_its_reason(tmp_path):
    ...
    assert any(v.rule == "claim.ordering_unchecked.no_run_record"
               for v in publication.warnings)
...
def test_not_applicable_with_a_reason_is_accepted():
    claim = Claim(id="c", text="t", registration=Registration.NOT_APPLICABLE,
                  registration_note="exhaustive deterministic count")
    assert claim.confirmatory is False


@pytest.mark.parametrize("flag,expected", [(True, Registration.CONFIRMATORY),
                                           (False, Registration.EXPLORATORY)])
...
def test_a_false_boolean_does_not_become_not_applicable():
    ...
    assert Claim(id="c", text="t", confirmatory=False).registration is not \
