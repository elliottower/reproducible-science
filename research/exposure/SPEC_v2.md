...
| Agent saw results accidentally, was terminated, no downstream edge | confirmatory, quarantine event | exposure isolated before propagation |
...
7. **The human rule.** If outcome information reached a human who continues making
   discretionary choices, quarantine cannot establish non-propagation.
...
neither produces this record nor can edit it. A `PreToolUse` variant blocks reads of
declared outcome paths outright, making prevention the default and the log the fallback.

An executable form of §4 and §6 — declaring per-context read/write permissions and
recording exposures against them — belongs in this paper as a proposed protocol. It must
**not** enter the Evidence Contract yet: it introduces agent contexts, permissions,
exposure events, and downstream causality, which are new objects beyond claim
verification. Let real cases reveal the abstraction.

Three properties to state plainly:

- **The log is one-sided.** It establishes that a path *was* read. Bash extraction is
  heuristic, so it cannot establish that one was not. "This exposure occurred and here is
  what followed" is a strong claim; "no exposure occurred" is a weak one.
- **Append-only is load-bearing.** A log the operator can rewrite proves nothing. Hash-chain
  it, as `results` already does for run ledgers, and anchor timestamps externally —
  repository commits or a timestamping service — rather than self-reported clocks.
- **Coverage is a parameter.** Report what fraction of tool calls the hook observed.
