...
def tier1_swh(entry: dict) -> dict | None:
    ...
    return {"tier": 1, "kind": "code_swh", "identifier": swhid, "resolved": True,
...
def tier4_commit_before(entry: dict) -> dict | None:
    ...
    if not payload or not isinstance(payload, list) or not payload:
        return None
    commit = payload[0]
    return {"tier": 4, "kind": "commit_before_publication", "identifier": commit.get("sha", ""),
            "resolved": True, "detail": f"{owner}/{repo} until {until}"}
...
def tier5_head(entry: dict) -> dict | None:
    ...
    if not payload:
        return None
    return {"tier": 5, "kind": "default_branch_head", "identifier": payload.get("default_branch", ""),
            "resolved": True, "sensitivity_only": True,
            "detail": f"{owner}/{repo}"}
...
def resolve(entry: dict) -> dict:
    ...
    for resolver in (tier1_swh, tier2_archival_doi, tier4_commit_before, tier5_head):
        try:
            ...
            return {"tier": None, "kind": None, "identifier": "", "resolved": False,
                    "retrieval_status": "unreachable", "unreachable": unreachable}
...
def main() -> int:
    ...
    for i, entry in enumerate(articles, 1):
        ...
        records.append({"key": entry["key"], "doi": entry.get("doi", ""),
                        "year": entry.get("year", ""), "domain": entry.get("domain", ""),
                        **outcome})
        print(f"  [{i:2}/{len(articles)}] {entry['key'][:34]:36} "
              f"tier {outcome['tier']}  {outcome['retrieval_status']}")

    if not args.limit:
        OUT.write_text(json.dumps({"resolved_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
...
if __name__ == "__main__":
    import urllib.parse  # noqa: E402  (used by tier1_swh)
